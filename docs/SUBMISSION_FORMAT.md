...
**Scoring.** A single score over the whole Δq vector would be dominated by the focal product's own response, hiding the substitution structure — so the counterfactual task grades TWO numbers from ONE scenario (`sweep_single_share_highest_plus10`, the flagship: a +10% rise in the focal product's effective price, realized by reducing its promotion depth in the store-weeks where it is on promotion), both from the *same* submitted Δq̂. The regular-price move on the same focal, shelf price +10% in every store-week, ships as `sweep_single_share_highest_regular_plus10` and is scored but not ranked. Both sides are first **category-netted** (each netted by its own category shift `Δq − ΔM·share`, `ΔM = Σ Δq`) so a category-wide magnitude move can't masquerade as substitution:
